    ...
    <div className="grid grid-cols-12 gap-6 h-full min-h-0 animate-in fade-in duration-300">
      ...
      <div className="col-span-4 flex flex-col gap-6 h-full min-h-0">
          ...
          <div className="p-2 lg:p-3 grid grid-cols-3 grid-rows-2 gap-2 lg:gap-3 flex-1 min-h-0">
            {ministers.map((minister, i) => (
              ...
              >
                ...
                <div className={`w-12 h-12 lg:w-14 lg:h-14 my-1 rounded-full flex items-center justify-center ${minister.color} border-2 lg:border-4 border-white shadow-md text-2xl lg:text-3xl transition-colors shrink-0`}>
                    {minister.status === 'happy' && '😊'}
                    {minister.status === 'neutral' && '😐'}
                    {minister.status === 'angry' && '😠'}
                </div>
                <div className="h-4 lg:h-5 flex items-center justify-center mt-1 lg:mt-2 shrink-0">
                  {selectedPolicy && Math.abs(minister.policyDelta) > 0.0005 ? (
                    <span className={`text-[10px] lg:text-xs font-black px-2 py-0.5 rounded-full shadow-sm ${minister.policyDelta > 0 ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-100 text-rose-700'}`}>
                      {minister.policyDelta > 0 ? '↑' : '↓'} {(Math.abs(minister.policyDelta) * 100).toFixed(0)}%
                    </span>
                  ) : null}
                </div>
              </div>
        ...
        <div onClick={() => setActiveTab('electorate')} className="bg-zinc-900 rounded-xl shadow-lg p-5 flex flex-col items-center justify-center shrink-0 h-40 relative overflow-hidden cursor-pointer hover:bg-black transition-colors group">
          ...
          <p className="text-xs font-bold uppercase tracking-widest text-zinc-400 mb-1">Public Approval</p>
