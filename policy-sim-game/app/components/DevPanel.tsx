...
export default function DevPanel({
  devMode, setDevMode, jumpToCycle, setCurrentTurn, currentTurn, turnsPerCycle,
  showOptimalPath, setShowOptimalPath, optimalPath, cycleMAO
}: DevPanelProps) {
  return (
    <>
      <button 
      ...
      {devMode && (
        <div className="fixed bottom-14 left-4 z-50 bg-zinc-900/95 backdrop-blur-md text-white p-5 rounded-2xl shadow-2xl border border-zinc-700 w-72 text-sm flex flex-col gap-4 animate-in fade-in slide-in-from-bottom-4">
          ...
          <div className="flex flex-col gap-2">
            ...
            <div className="grid grid-cols-2 gap-2">
              <button onClick={() => setCurrentTurn(prev => Math.min(turnsPerCycle, prev + 1))} className="bg-zinc-800 hover:bg-zinc-700 px-3 py-2 rounded-lg text-xs font-bold transition-colors">Skip 1 Turn</button>
              <button onClick={() => setCurrentTurn(turnsPerCycle)} className="bg-zinc-800 hover:bg-zinc-700 px-3 py-2 rounded-lg text-xs font-bold transition-colors">Jump to End</button>
            </div>
          </div>
