...
export const ModalOverlay = ({ children }: { children: React.ReactNode }) => (
  <motion.div 
    initial={{ opacity: 0, x: -80, y: 15, rotate: -2 }}
    animate={{ opacity: 1, x: 0, y: 0, rotate: 0 }}
    exit={{ opacity: 0, x: 80, y: 15, rotate: 2 }}
...
export const ModalContent = ({ children, maxWidth = "max-w-4xl" }: { children: React.ReactNode, maxWidth?: string }) => (
  <motion.div 
    ...
    className={`bg-white rounded-2xl shadow-2xl w-full ${maxWidth} flex flex-col border-x border-zinc-200 border-t-[6px] border-t-pink-600 border-b-[6px] border-b-zinc-900 max-h-[95vh]`}
  >
...
export const InteractiveDPMEmail = ({ 
  ...
  onAcknowledge,
  buttonText = "Acknowledge & Unlock Agenda"
}: InteractiveDPMEmailProps) => {
  const [isOpen, setIsOpen] = useState(false);
  const [buttonUnlocked, setButtonUnlocked] = useState(false);
  
  const { displayedText, isComplete } = useTypewriter(message, typeSpeed, isOpen);
  ...
  const handleAcknowledge = () => {
    // 1. Fold the message back into the folder button
    setIsOpen(false);
    
    // 2. Wait exactly for the fold animation to finish (500ms), 
    // then tell the parent to fly the entire modal away.
    setTimeout(() => {
      onAcknowledge();
    }, 500); 
  };
  ...
  return (
    // The layout prop here ensures the container smoothly animates its height
    <motion.div layout className="w-full relative flex flex-col">
      {/* popLayout makes the exiting element absolute so the height can transition seamlessly */}
      <AnimatePresence mode="popLayout" initial={false}>
        {!isOpen ? (
          ...
          <motion.div 
            key="message"
            layout
            // Added a subtle blur effect for a premium crossfade feel
          ...
          >
            <DPMMessage title={title}>
              <div className="relative">
                {/* Ghost text establishes the final height immediately without being seen */}
                <span className="whitespace-pre-wrap invisible block" aria-hidden="true">
                  {message}
                </span>
                
                {/* Actual text overlays the ghost text */}
                <span className="whitespace-pre-wrap absolute top-0 left-0 w-full h-full">
                  {displayedText}
                  {!isComplete && <span className="inline-block w-1.5 h-4 ml-1 bg-zinc-400 animate-pulse" />}
                </span>
              </div>
            </DPMMessage>
            ...
            <button 
              ...
              className={`w-full py-4 text-sm font-bold uppercase tracking-widest rounded-xl transition-all shadow-md shrink-0 duration-500 ${
                buttonUnlocked 
                  ? 'bg-zinc-900 text-white hover:bg-black translate-y-0 opacity-100 cursor-pointer' 
                  : 'bg-zinc-200 text-zinc-400 cursor-not-allowed translate-y-2 opacity-50'
              }`}
            >
              {buttonText}
            </button>
