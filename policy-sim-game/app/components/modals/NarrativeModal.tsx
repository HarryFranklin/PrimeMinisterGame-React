  ...
  const renderContent = () => {
    switch (completedCycle) {
          ...
          <>
            ...
            {rawlsExplanation && (
              <div className="animate-in fade-in slide-in-from-bottom-4 flex flex-col gap-4">
                ...
                <ModalActionBtn onClick={onProceed}>Restart Simulation: Cycle 3 (Societal Utility)</ModalActionBtn>
              </div>
            )}
          </>
        );
      ...
      case ElectionCycle.SocietalUtility: {
        ...
        return (
          <>
            <ModalHeader title="The Collective vs The Individual" />
            
            <DPMMessage title="Personal Evaluation">
              "Societal Utility models citizens evaluating the fairness of the entire distribution. Click on the citizen below to reveal how their perspective shifts when accounting strictly for their own self-interest."
            </DPMMessage>
            ...
            <div className="w-full">
              ...
              >
                ...
                <div className="grid grid-cols-2 gap-4 mb-2">
                  ...
                  <div>
                    <span className="text-xs text-zinc-400 block mb-1">Societal Utility</span>
                    <strong className="text-2xl text-zinc-800">{su.toFixed(2)}</strong>
                  </div>
                </div>
                
                <div className={`transition-all duration-500 ${revealedEmpathy ? 'opacity-100 transform-none' : 'opacity-0 translate-y-4 hidden'}`}>
                  <div className="w-full h-px bg-zinc-200 my-3" />
                  <span className="text-xs text-emerald-500 font-bold uppercase tracking-widest block mb-1">Personal Utility (Self-Interest)</span>
                  <strong className="text-3xl text-emerald-600">{pu.toFixed(2)}</strong>
                  <p className="text-[10px] text-zinc-500 mt-2 max-w-sm mx-auto italic">
                    "While my evaluation of society drops due to inequality, my personal score is significantly higher when evaluating strictly for myself."
                  </p>
                </div>

                {!revealedEmpathy && (
                  <div className="absolute inset-0 flex items-center justify-center bg-zinc-900/5 backdrop-blur-[1px] opacity-0 group-hover:opacity-100 transition-opacity">
                    <span className="bg-white px-4 py-2 rounded-full text-xs font-bold shadow-sm text-emerald-600">Reveal Personal Utility</span>
                  </div>
                )}
              </div>
            </div>
            ...
            {revealedEmpathy && !personalExplanation && (
              <div className="animate-in fade-in slide-in-from-bottom-4 flex flex-col gap-4">
                <DPMMessage title="Observation">
                  "Notice the upward adjustment. Shall we review why this occurs?"
                </DPMMessage>
            ...
            {personalExplanation && (
              <div className="animate-in fade-in slide-in-from-bottom-4 flex flex-col gap-4">
                <DPMMessage title="The Empathy Trap" className="border-emerald-200 bg-emerald-50/30">
                  "When citizens evaluate policy based on empathy, consensus is difficult because everyone has a different definition of fairness. For your final term, we will incorporate <strong>Personal Utility</strong> into their voting logic, modelling pure self-interest."
                </DPMMessage>
                <ModalActionBtn onClick={onProceed}>Restart Simulation: Final Cycle (Personal Utility)</ModalActionBtn>
              </div>
            )}
          </>
        );
      ...
      case ElectionCycle.PersonalUtility:
        return (
          <>
            <ModalHeader title="Societal vs. Personal Utility" />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="bg-emerald-50 rounded-xl border border-emerald-200 p-5 flex flex-col">
                <h3 className="text-xs font-bold uppercase tracking-widest text-emerald-800 mb-2 text-center">Cycle 3: Societal Utility</h3>
              ...
              <div className="bg-zinc-50 rounded-xl border border-zinc-200 p-5 flex flex-col">
                <h3 className="text-xs font-bold uppercase tracking-widest text-zinc-800 mb-2 text-center">Term 4: Personal Utility</h3>
                <div className="text-center mb-3">
