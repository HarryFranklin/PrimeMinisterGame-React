...
import { PERSONAL_UTILITY_TABLE, personalUtility } from '../../lib/utility';

/**
 * Shared Personal Utility curve, drawn as plain SVG so it can be highlighted,
 * hovered and given a moving marker. Used by UtilityCurveDiagram and
 * UtilityInterventionWidget so both always show the same curve.
 *
 * The x-axis starts at Life Satisfaction 2, matching lib/utility.ts
 * (scores of 2 or below have 0 utility).
 */
...
const segmentPath = (from: number, to: number) =>
  knots(from, to)
    .map((x, i) => `${i === 0 ? 'M' : 'L'} ${scaleX(x)} ${scaleY(personalUtility(x))}`)
    .join(' ');
...
export default function UtilityCurveChart({
  ...
  onZoneLeave,
  onZoneClick,
  ariaLabel = 'Personal Utility curve',
}: UtilityCurveChartProps) {
  const curvePoints = Object.entries(PERSONAL_UTILITY_TABLE).map(([ls, u]) => ({ ls: Number(ls), u }));
  ...
  return (
    <svg viewBox={`0 0 ${WIDTH} ${HEIGHT}`} className="w-full h-auto select-none" role="img" aria-label={ariaLabel}>
      ...
      {steps.map((s, i) => {
        ...
        const y1 = scaleY(personalUtility(s.from));
        const y2 = scaleY(personalUtility(s.to));
      ...
      {/* Moving marker (used by the slider widget) */}
      {marker !== null && (
        <g>
          <line x1={scaleX(marker)} x2={scaleX(marker)} y1={scaleY(personalUtility(marker))} y2={axisY} stroke="#a1a1aa" strokeWidth={1.5} strokeDasharray="3 3" />
          <circle cx={scaleX(marker)} cy={scaleY(personalUtility(marker))} r={9} fill="#18181b" stroke={markerColor} strokeWidth={4} />
        </g>
      )}
